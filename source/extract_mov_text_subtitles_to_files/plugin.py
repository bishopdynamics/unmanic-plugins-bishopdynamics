...
from extract_mov_text_subtitles_to_files.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.extract_mov_text_subtitles_to_files")
...
class PluginStreamMapper(StreamMapper):
    ...
    def test_stream_needs_processing(self, stream_info: dict):
        ...
        if stream_info.get('codec_name', '').lower() not in ['mov_text']:
            return False
...
def mov_text_already_extracted(settings, path):
    directory_info = UnmanicDirectoryInfo(os.path.dirname(path))

    try:
        already_extracted = directory_info.get('extract_mov_text_subtitles_to_files', os.path.basename(path))
    ...
    if already_extracted:
        logger.debug("File's mov_text subtitle streams were previously extracted with {}.".format(already_extracted))
        return True
...
def on_library_management_file_test(data):
    ...
    if not mov_text_already_extracted(settings, abspath):
        # Mark this file to be added to the pending tasks
        data['add_file_to_pending_tasks'] = True
        logger.debug("File '{}' should be added to task list. File has not been previously had mov_text/94213 extracted.".format(abspath))
    else:
        logger.debug("File '{}' has been previously had mov_text/94213 extracted.".format(abspath))
...
def on_worker_process(data):
    ...
    if not mov_text_already_extracted(settings, data.get('file_in')):
        # Get stream mapper
        mapper = PluginStreamMapper()
...
def on_postprocessor_task_results(data):
    ...
    for destination_file in data.get('destination_files'):
        ...
        directory_info.set('extract_mov_text_subtitles_to_files', os.path.basename(destination_file), subs)
        directory_info.save()
        logger.info("mov_text subtitles processed for '{}' and recorded in .unmanic file.".format(destination_file))
